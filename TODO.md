...
## 已完成
- [x] 优化鼠标准心抖动与拉伸效果，增强移动/开火 gamefeel
- [x] 将 `Level:randomizeEdges()` 的规则拆到 `src/level_edge_rules.lua`，降低 `Level` 分支复杂度
- [x] 限制同屏敌人上限并平滑刷怪批量，缓解高波次卡顿
...
## 验证记录
- 已新增 `tests/cursor_effects_spec.lua`，覆盖准星移动拉伸、开火脉冲与衰减
- 已新增 `tests/level_edge_rules_spec.lua`，覆盖传送门布局与刷怪边保底规则
- 已通过 LÖVE 临时启动器再次执行测试，结果为 `tests passed: 31`
- 已再次执行 `timeout 5s love .` 冒烟启动，未出现即时启动报错（进程因超时退出码 `124` 结束，属预期）
- 已新增波次/世界测试，覆盖敌人同屏上限、单帧刷怪批量与 tag 计数逻辑
...
## 后续建议
- 若还想继续加强准星反馈，可再叠加命中闪白或击杀时的准星外圈脉冲
- 下一步可继续把 `Level:randomizeEdges()` 的实例化部分再下沉到边工厂，进一步缩短 `Level`
