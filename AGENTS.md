...
## Lua Annotation Guidelines
All new or modified Lua functions and methods must document their explicit parameters with EmmyLua annotations immediately before the definition, using `---@param name type` for every parameter. Method definitions using `function Class:method(arg)` do not need a `self` annotation because `self` is implicit, but every explicit argument such as `arg` must be annotated. Add `---@return` when the return value is consumed by callers or useful for tests/debugging. When touching an existing function that lacks annotations, add the missing parameter annotations as part of the same change instead of leaving the signature undocumented.
